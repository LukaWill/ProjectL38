...
Metabolite <- read.delim("max_5.txt", comment.char = "#")
...
Metabolite_select_up$MaxUTR<-as.numeric(Metabolite_select_up$max_5_UTR)

Randomup <- read.delim("Random_592.txt", comment.char="#")

select_up<- which(Metabolite$GeneID%in%Randomup$GeneID)
Metabolite_select_upRandom <- Metabolite[select_up,]
Metabolite_select_upRandom$MaxUTR<-as.numeric(Metabolite_select_upRandom$max_5_UTR)
...
Metabolite_select_down$MaxUTR<-as.numeric(Metabolite_select_down$max_5_UTR)

Randomdown <- read.delim("Random_823.txt", comment.char="#")

select_down<- which(Metabolite$GeneID%in%Randomdown$GeneID)
Metabolite_select_downRandom <- Metabolite[select_down,]
Metabolite_select_downRandom$MaxUTR<-as.numeric(Metabolite_select_downRandom$max_5_UTR)
...
pdf("5_UTR_numbers.pdf")
plot(DEGs_up$DEGs_up, col="orange", pch=19,
     main="5'UTR numbers",
     xlab="5'UTR length",
     ylab="Number of transcripts with 5'UTRs", xaxt = "n",
     ylim=c(0,600))
...
legend('topright',legend=c("DEGs_up", "Random_up","DEGs_down","Random_down"), pch=c(19, 19), col=c('orange', 'blue','green', 'red'))
dev.off()