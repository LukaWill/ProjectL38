...
######################################################################
# Main directory:
setwd("~/Project_bioinformatics") #Full path of the working directory. 
#It must contain: 
...
## Metadata options
metaFile <- "RNA_RIBO_meta.csv" #Name of metadata file in csv format.
doFilter <- F #F
#whichFilter <- c("Riboseq_control_1", "Riboseq_control_2", "Riboseq_control_3", "Riboseq_sample_1", "Riboseq_sample_2", "RNAseq_control_1", "RNAseq_control_2", "RNAseq_control_3", "RNAseq_sample_1", "RNAseq_sample_2", "RNAseq_sample_3") #If there are libraries that need to be filtered out (Avoid removing columns manually from the raw counts file)(single end data is not taken used)

## Counts file name (with extension) 
countsFile <- "RNA_RIBO_control_sample_counts.txt" #Name of metadata file

shortName <- "" #Short name to append at the end of the filenames. If missing it will append the name of the folder where the scripts where run.
...
## Output
outDir = "RNA-RIBO-interaction/"
dir.create(outDir, showWarnings=T)

 sink('RNA-RIBO-interaction/Output.txt')

geneListsDir = "RNA-RIBO-interaction/GeneLists"
dir.create(geneListsDir, showWarnings=T)
#
imgDir = "RNA-RIBO-interaction/images/"
...
Groups <- as.factor(paste0(experimentFactors$Population,"_",experimentFactors$Genotype))
design <- model.matrix(~0+Groups) 
# Example of an interaction
#design <- model.matrix(~0+experimentFactors$Sample*experimentFactors$Population) #Sample*Population interaction
...
cont.matrix= makeContrasts(
  ...
  "Riboseq_sample-Riboseq_control"=Riboseq_sample-Riboseq_control,
  "RNAseq_sample-RNAseq_control"=RNAseq_sample-RNAseq_control,
  "Riboseq_control-RNAseq_control"=Riboseq_control-RNAseq_control,
  "Riboseq_sample-RNAseq_sample"=Riboseq_sample-RNAseq_sample,
  levels=design)
